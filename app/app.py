...
def add_patient():
    if request.method == 'POST':
        ...
        cursor = conn.cursor()
        cursor.execute("INSERT INTO Patients (name, lastName,document,telf,date,email,password) VALUES (?, ?, ?, ?, ?, ?, ?)", (nombre, apellido,documento,telefono,date,correo,contrasena ))
        conn.commit()
        conn.close()
...
def add_doctor():
    if request.method == 'POST':
        ...
        conn = sqlite3.connect(database_path)
        cursor = conn.cursor()
        cursor.execute("INSERT INTO Doctors (name, lastName, description) VALUES ( ?, ?, ?)", (nombre, apellido,descripcion ))
        conn.commit()
        conn.close()
...
def login_patient():
    try:
        ...
        if request.method == 'POST':
            ...
            if paciente:
                return redirect(url_for('predict_section', paciente_id=paciente[0]))
...
def predict_section(paciente_id):
    ...
    if result:
        ...
        return render_template('predict_section.html', paciente_name=nombre,paciente_id=paciente_id)
    else:
        abort(404)
...
def upload_image(paciente_id):
    ...
    if file and allowed_file(file.filename):
        ...
        image = cv2.imdecode(np.fromstring(uploaded_image, np.uint8), cv2.IMREAD_UNCHANGED)
        ...
        if predicted_class == "PSORIASIS":
            conn = sqlite3.connect(database_path)
            cursor = conn.cursor()
            cursor.execute('UPDATE Patients SET disease = ? WHERE id = ?', (predicted_class, paciente_id))
            conn.commit()
            conn.close()
            return render_template('case_psoriasis.html',mensaje=predicted_class,image=encoded_image)
        if predicted_class == "ROSACEA":
            ...
            conn.commit()
            conn.close()
            return render_template('case_rosacea.html',mensaje=predicted_class,image=encoded_image)
        if predicted_class == "SARPULLIDO":
            ...
            conn.commit()
            conn.close()
            return render_template('case_sarpullido.html',mensaje=predicted_class,image=encoded_image)
        if predicted_class == "VITILIGIO":
            ...
            conn.commit()
            conn.close()
            return render_template('case_vitiligio.html',mensaje=predicted_class,image=encoded_image)        
        return render_template('index.html')
